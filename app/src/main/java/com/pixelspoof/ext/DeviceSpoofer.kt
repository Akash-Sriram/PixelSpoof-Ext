...
object DeviceSpoofer {
    ...
    fun hook(module: XposedModule?, allowFailureUi: Boolean = true) {
        val verboseLog = false
        val deviceName = "Pixel XL"
        Log.d(TAG, "Device spoof: $deviceName (sdk=${Build.VERSION.SDK_INT})")

        val deviceEntries = DeviceProps.getDeviceProps(deviceName)
        if (deviceEntries == null || deviceEntries.props.isEmpty()) {
    ...
    private fun buildSystemPropertyOverrides(props: Map<String, String>): Map<String, String> {
        ...
        putKey("ro.product.vendor_dlkm.brand", props["BRAND"])
        putKey("ro.product.vendor_dlkm.device", props["DEVICE"])
        putKey("ro.product.vendor_dlkm.model", props["MODEL"])
        putKey("ro.product.vendor_dlkm.name", props["PRODUCT"])
        putKey("ro.product.vendor_dlkm.manufacturer", props["MANUFACTURER"])
